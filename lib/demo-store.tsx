...
import type { Avatar, Classroom, Student } from "./types";
import { WebMcpTools } from "./webmcp";

const KEY = "chemclass-demo-v1";
const SESSION = "chemclass-student-session";
const defaultAvatar: Avatar = { skin: "medium", hair: "short", hairColor: "black", shirt: "cyan" };

function desks(count = 12) { return Array.from({length: count}, (_, i) => ({ id: `desk-${i+1}`, label: `${i+1}`, locked: false, x: i % 4, y: Math.floor(i / 4) })); }
const seed: Classroom[] = [{ id: "demo-room", code: "CHEM82", name: "ห้องทดลอง ม.2/1", subject: "วิทยาศาสตร์ — การแยกสาร", layout: "ROWS", status: "OPEN", desks: desks(), students: [], createdAt: new Date().toISOString() }];
...
export function DemoProvider({children}:{children:React.ReactNode}) {
  ...
  useEffect(() => { try { const saved=localStorage.getItem(KEY); if(saved) setRooms(JSON.parse(saved)); const session=localStorage.getItem(SESSION); if(session) setCurrentStudent(JSON.parse(session)); } finally { setReady(true); } }, []);
  useEffect(() => { if(ready) localStorage.setItem(KEY,JSON.stringify(rooms)); },[rooms,ready]);
  useEffect(() => { const sync=(event:StorageEvent)=>{if(event.key===KEY&&event.newValue)setRooms(JSON.parse(event.newValue));}; addEventListener("storage",sync); return()=>removeEventListener("storage",sync); },[]);
  const mutate = useCallback((fn:(rooms:Classroom[])=>Classroom[])=>setRooms(old=>fn(structuredClone(old))),[]);
  const createRoom:Store["createRoom"] = input => { const room:Classroom={id:crypto.randomUUID(),code:Math.random().toString(36).slice(2,8).toUpperCase(),name:input.name,subject:input.subject,layout:input.layout,status:"OPEN",desks:desks(input.count),students:[],createdAt:new Date().toISOString()}; setRooms(r=>[room,...r]); return room; };
  ...
  const recordAttempt=(levelId:number,correct:boolean,score:number)=>currentStudent&&mutate(rs=>rs.map(r=>r.id===currentStudent.roomId?{...r,students:r.students.map(s=>s.id===currentStudent.studentId?{...s,wrongAttempts:s.wrongAttempts+(correct?0:1),totalScore:s.totalScore+(correct&&!s.completed.includes(levelId)?score:0),completed:correct?[...new Set([...s.completed,levelId])]:s.completed,currentLevel:correct?Math.min(8,levelId+1):s.currentLevel}:s)}:r));
  const value={ready,rooms,currentStudent,createRoom,join,updateAvatar,selectDesk,toggleHand,setRoomStatus,setDeskLock,setAllLocks,moveStudent,recordAttempt};
