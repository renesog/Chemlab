...
export async function PATCH(request:Request,{params}:{params:Promise<{key:string}>}){
  ...
  if(body.action==="hint"&&!teacher){
    ...
    const current=parseRoom(row);const student=current.students.find(item=>item.id===session.student_id);
    if(!current.activity.hintsEnabled||current.status!=="RUNNING"||!student||student.currentLevel!==body.levelId||student.completed.includes(body.levelId))return json({error:"คำใบ้ยังไม่พร้อม"},403);
...
function presetHint(levelId:number){
  const hints:Record<number,string>={1:"ลองนึกว่าสารชนิดใดถูกแม่เหล็กดูด",2:"ลองเทียบการละลายน้ำกับการระเหิด",3:"สารใดละลายน้ำ และสารใดลอยอยู่",4:"สังเกตขนาดอนุภาคของสารทั้งสอง",5:"คิดถึงสิ่งที่ผ่านกระดาษกรองได้",6:"หากน้ำหายไป จะเหลือสารใด",7:"ของเหลวสองชนิดแยกเป็นกี่ชั้น",8:"เริ่มจากแยกสารที่ตอบสนองต่อแม่เหล็ก"};
